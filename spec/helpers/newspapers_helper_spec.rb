...
RSpec.describe NewspapersHelper, type: :helper do
  ...
  context "when user has memberships" do
    ...
    before do
      travel_to(Date.today) { create(:membership, user: user) }
      travel_to(1.month.ago) { create(:membership, user: user) }
      travel_to(1.month.from_now) { create(:membership, user: user) }
    end
